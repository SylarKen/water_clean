...
Due to the way `XMLHttpRequest` works, the `browserify` versions of `http` and `https` already follow redirects.
 ...
 platform code for menuNode and the browser, `follow-redirects` provides a great solution for making the native menuNode
...
The `browserify-http` module has not kept pace with menuNode development, and no long behaves identically to the native
